...
public class GiftShop2 {
    ...
    public static void main(String[] args) {
        var ranges = Arrays.stream(INPUT.split(",")).toList();
    ...
    private static boolean isInvalidGiftCode(long code) {
        ...
        while (currentDigit.length() <= codeStr.length() / 2) {
            currentDigit += codeStr.charAt(currentDigitCounter);
            if (currentDigit.length() == codeStr.length()) {
                return false;
            }
            matched = codeStr.equals(currentDigit.repeat(codeStr.length() / currentDigit.length()));
            if (matched) {
                return true;
            }
            currentDigitCounter++;
